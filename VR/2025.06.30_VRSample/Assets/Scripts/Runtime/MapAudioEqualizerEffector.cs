...
public class MapAudioEqualizerEffector : MonoBehaviour
{
    ...
    private void Update()
    {
        _source.GetSpectrumData(_spectrumDatum, 0, FFTWindow.Hamming);
        ...
        for (int i = 1; i < _spawnCount - 1; i++)
        {
            ...
            _spawnedUnits[i].localScale = new Vector3(1, Mathf.Lerp(prevScaleY, _spectrumDatum[i], Time.deltaTime * 30f), 1);
        }
    }
}
