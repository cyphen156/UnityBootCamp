...
namespace _25_06_04_AR_First.Mapping
{
    ...
    /// GPS 데이터가 범위를 벗어날 때 타일맵 확장 및 반대방향 타일맵 삭제
    /// </summary>
    public class GoogleMapTileManager : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            yield return null; // 한 프레임 대기 (타일이 완전히 생성되도록)

            foreach (var tile in _mapTiles)
            {
                tile.RefreshMapTile(); // 한 번 더 강제 갱신
            }
            ...
            for (int i = 0; i < TILE_OFFSETS.Length; i++)
            {
                for (int j = 0; j < TILE_OFFSETS.Length; j++)
                {
                    Vector2Int coord = new Vector2Int(
                        TILE_OFFSETS[i] + center.x,
                        TILE_OFFSETS[j] + center.y
                        );
                    ...
                    tile.name = $"MapTile {coord.x}_{coord.y}";
                    tile.tileOffset = new Vector2Int(i - 1, j - 1);
                    tile.transform.position = CalcTileWorldPosition(coord);
                    tile.RefreshMapTile();
                    _mapTiles[i, j] = tile;
                }
            }
        ...
        private Vector3 CalcTileWorldPosition(Vector2Int coord)
        {
        ...
        private Vector2Int CalcTileCoordinate(MapLocation center)
        {
        ...
        private void ShiftHorizontal(int dir)
        {
            ...
            int newX = dir > 0 ? GRID_SIZE - 1 : 0;    // 생성할 인덱스
            ...
            for (int x = 0; x < GRID_SIZE; x++)
            {
                for (int y = 0; y < GRID_SIZE; y++)
                {
                    ...
                    tile.transform.position = CalcTileWorldPosition(_currentCenterTileCoord + offset);
                    if (x == newX)
                    {
                        tile.RefreshMapTile();
                    }
        ...
        private void ShiftVertical(int dir)
        {
            ...
            int newY = dir > 0 ? GRID_SIZE - 1 : 0;    // 생성할 인덱스
